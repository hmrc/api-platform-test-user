...
class GeneratorSpec extends UnitSpec with MockitoSugar with PropertyChecks {
  ...
  "generateTestIndividual" should {
    ...
    "generate an EORI when ICS_SAFETY_AND_SECURITY service is included" in new Setup {
      ...
      val individual = await(underTest.generateTestIndividual(Seq(ICS_SAFETY_AND_SECURITY)))
  ...
  "generateTestOrganisation" should {
    ...
    "generate an EORI when ICS_SAFETY_AND_SECURITY service is included" in new Setup {
      ...
      val org = await(underTest.generateTestOrganisation(Seq(ICS_SAFETY_AND_SECURITY)))
    ...
    "generate an EORI when SAFETY_AND_SECURITY service is included" in new Setup {
      ...
      val org = await(underTest.generateTestOrganisation(Seq(SAFETY_AND_SECURITY)))
