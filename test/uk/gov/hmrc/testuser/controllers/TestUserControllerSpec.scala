...
class TestUserControllerSpec extends AsyncHmrcSpec with LogSuppressing {
  ...
  val orgProps = Map[TestUserPropKey, String](
    ...
    TestUserPropKey.zReference                              -> zReference.value
  )
  ...
  val createDisaOrganisationServices    = Seq(DISA)
  ...
  trait Setup {
    ...
    def createOrganisationWithZReference = {
      val jsonPayload: JsValue = Json.parse(
        s"""{"serviceNames":["disa"], "zReference": "${zReference.value}"}"""
      )
      FakeRequest().withBody[JsValue](jsonPayload)
    }
  ...
  "createOrganisation" should {
    ...
    "return 201 (Created) with the created organisation" in new Setup {
      ...
      when(underTest.testUserService.createTestOrganisation(eqTo(createOrganisationServices), eqTo(None), eqTo(None), eqTo(None), eqTo(None), eqTo(None), eqTo(None))(*))
    ...
    "return 201 (Created) with the created organisation with provided zReference" in new Setup {

      when(underTest.testUserService.createTestOrganisation(
        eqTo(createDisaOrganisationServices),
        eqTo(None),
        eqTo(None),
        eqTo(None),
        eqTo(None),
        eqTo(None),
        eqTo(Some(zReference))
      )(*)).thenReturn(successful(Right(testOrganisation)))

      val result = underTest.createOrganisation()(createOrganisationWithZReference)
      status(result) shouldBe CREATED
    }

    "fail with 500 (Internal Server Error) when the creation of the organisation failed" in new Setup {
      when(underTest.testUserService.createTestOrganisation(*, *, eqTo(None), eqTo(None), eqTo(None), eqTo(None), eqTo(None))(*))
        .thenReturn(failed(new RuntimeException("expected test error")))
