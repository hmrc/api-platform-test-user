...
object ServiceKeys extends Enumeration {
  ...
  val SECURE_ELECTRONIC_TRANSFER: ServiceKeys.Value = Value(
  ...
  val GOODS_VEHICLE_MOVEMENTS: ServiceKeys.Value = Value(
    "goods-vehicle-movements")
  val ICS_SAFETY_AND_SECURITY: ServiceKeys.Value = Value(
    "ics-safety-and-security")
}

case class Service(key: ServiceKey,
                   name: String,
                   allowedUserTypes: Seq[UserType])

object Services extends Seq[Service] {
  val services = Seq(
    Service(ServiceKeys.NATIONAL_INSURANCE,
            "National Insurance",
            Seq(INDIVIDUAL, ORGANISATION)),
    Service(ServiceKeys.SELF_ASSESSMENT,
            "Self Assessment",
            Seq(INDIVIDUAL, ORGANISATION)),
    Service(ServiceKeys.CORPORATION_TAX, "Corporation Tax", Seq(ORGANISATION)),
    Service(ServiceKeys.PAYE_FOR_EMPLOYERS,
            "PAYE for Employers",
            Seq(ORGANISATION)),
    Service(ServiceKeys.SUBMIT_VAT_RETURNS,
            "Submit VAT Returns",
            Seq(ORGANISATION)),
    Service(ServiceKeys.MTD_VAT, "MTD VAT", Seq(INDIVIDUAL, ORGANISATION)),
    Service(ServiceKeys.MTD_INCOME_TAX,
            "MTD Income Tax",
            Seq(INDIVIDUAL, ORGANISATION)),
    Service(ServiceKeys.AGENT_SERVICES, "Agent Services", Seq(AGENT)),
    Service(ServiceKeys.LISA, "Lifetime ISA", Seq(ORGANISATION)),
    Service(ServiceKeys.SECURE_ELECTRONIC_TRANSFER,
            "Secure Electronic Transfer",
            Seq(ORGANISATION)),
    Service(ServiceKeys.RELIEF_AT_SOURCE,
            "Relief at Source",
            Seq(ORGANISATION)),
    Service(ServiceKeys.CUSTOMS_SERVICES,
            "Customs Services",
            Seq(INDIVIDUAL, ORGANISATION)),
    Service(ServiceKeys.GOODS_VEHICLE_MOVEMENTS,
            "Goods Vehicle Services",
            Seq(INDIVIDUAL, ORGANISATION)),
    Service(ServiceKeys.ICS_SAFETY_AND_SECURITY,
            "ICS Safety and Security",
            Seq(INDIVIDUAL, ORGANISATION))
  )
...
object FetchTestIndividualResponse {
  def from(individual: TestIndividual) =
    FetchTestIndividualResponse(
      individual.userId,
...
object TestAgentCreatedResponse {
  def from(agent: TestAgent) =
    TestAgentCreatedResponse(agent.userId,
                             agent.password,
                             agent.userFullName,
                             agent.emailAddress,
                             agent.arn,
...
case class DesSimulatorTestIndividual(mtdItId: Option[String],
                                      vrn: Option[String],
                                      nino: Option[String],
                                      saUtr: Option[String])
...
object DesSimulatorTestIndividual {
  def from(individual: TestIndividual) =
    DesSimulatorTestIndividual(individual.mtdItId,
                               individual.vrn,
                               individual.nino,
                               individual.saUtr)
}

case class DesSimulatorTestOrganisation(mtdItId: Option[String],
                                        nino: Option[String],
                                        saUtr: Option[String],
                                        ctUtr: Option[String],
...
object DesSimulatorTestOrganisation {
  def from(organisation: TestOrganisation) =
    DesSimulatorTestOrganisation(organisation.mtdItId,
                                 organisation.nino,
                                 organisation.saUtr,
                                 organisation.ctUtr,
                                 organisation.empRef,
...
object MtdItId extends Modulus23Check with (String => MtdItId) {
  implicit val mtdItIdWrite: Writes[MtdItId] =
    new SimpleObjectWrites[MtdItId](_.value)
  implicit val mtdItIdRead: Reads[MtdItId] =
    new SimpleObjectReads[MtdItId]("mtdItId", MtdItId.apply)
  implicit val mtdItIdFormat: Format[MtdItId] =
    Format(mtdItIdRead, mtdItIdWrite)
...
case class LisaManagerReferenceNumber(lisaManagerReferenceNumber: String)
...
object LisaManagerReferenceNumber
    extends (String => LisaManagerReferenceNumber) {
  implicit val lisaManRefNumWrite: Writes[LisaManagerReferenceNumber] =
    new SimpleObjectWrites[LisaManagerReferenceNumber](_.value)
  implicit val lisaManRefNumRead: Reads[LisaManagerReferenceNumber] =
    new SimpleObjectReads[LisaManagerReferenceNumber](
      "lisaManagerReferenceNumber",
      LisaManagerReferenceNumber.apply)
}

case class SecureElectronicTransferReferenceNumber(
    secureElectronicTransferReferenceNumber: String)
    extends TaxIdentifier
    with SimpleName {
...
object SecureElectronicTransferReferenceNumber
    extends (String => SecureElectronicTransferReferenceNumber) {
  implicit val secureElectronicTransferReferenceNumberWrite
    : Writes[SecureElectronicTransferReferenceNumber] =
    new SimpleObjectWrites[SecureElectronicTransferReferenceNumber](_.value)

  implicit val secureElectronicTransferReferenceNumberRead
    : Reads[SecureElectronicTransferReferenceNumber] =
    new SimpleObjectReads[SecureElectronicTransferReferenceNumber](
      "secureElectronicTransferReferenceNumber",
      SecureElectronicTransferReferenceNumber.apply)
}

case class PensionSchemeAdministratorIdentifier(
    pensionSchemeAdministratorIdentifier: String)
    extends TaxIdentifier
    with SimpleName {
...
object PensionSchemeAdministratorIdentifier
    extends (String => PensionSchemeAdministratorIdentifier) {
  implicit val pensionSchemeAdministratorIdentifierWrite
    : Writes[PensionSchemeAdministratorIdentifier] =
    new SimpleObjectWrites[PensionSchemeAdministratorIdentifier](_.value)

  implicit val pensionSchemeAdministratorIdentifierRead
    : Reads[PensionSchemeAdministratorIdentifier] =
    new SimpleObjectReads[PensionSchemeAdministratorIdentifier](
      "pensionSchemeAdministratorIdentifier",
      PensionSchemeAdministratorIdentifier.apply)
}

case class EoriNumber(override val value: String)
    extends TaxIdentifier
    with SimpleName {
