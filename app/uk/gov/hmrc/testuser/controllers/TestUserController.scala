...
import uk.gov.hmrc.testuser.models.ErrorResponse.{
...
class TestUserController @Inject()(
  ...
  def createIndividual() = Action.async(parse.json) { implicit request =>
    withJsonBody[CreateUserRequest] { createUserRequest =>
      testUserService.createTestIndividual(
        createUserRequest.serviceNames.getOrElse(Seq.empty)) map { individual =>
  ...
  def createOrganisation() = Action.async(parse.json) { implicit request =>
    withJsonBody[CreateUserRequest] { createUserRequest =>
      testUserService.createTestOrganisation(
        createUserRequest.serviceNames.getOrElse(Seq.empty)) map {
        organisation =>
  ...
  def createAgent() = Action.async(parse.json) { implicit request =>
    withJsonBody[CreateUserRequest] { createUserRequest =>
      testUserService.createTestAgent(
        createUserRequest.serviceNames.getOrElse(Seq.empty)) map { agent =>
  ...
  def fetchIndividualByShortNino(shortNino: NinoNoSuffix) = Action.async {
    implicit request =>
  ...
  def fetchOrganisationByEmpRef(empRef: EmpRef) = Action.async {
    implicit request =>
  ...
  private def recovery: PartialFunction[Throwable, Result] = {
    ...
    case UserNotFound(ORGANISATION) =>
      NotFound(toJson(organisationNotFoundError))
    case e =>
