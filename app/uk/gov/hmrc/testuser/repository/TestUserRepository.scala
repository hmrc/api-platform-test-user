...
class TestUserRepository @Inject()(mongo: ReactiveMongoComponent)(
    ...
    extends ReactiveRepository[TestUser, BSONObjectID](
      "testUser",
      mongo.mongoConnector.db,
      JsonFormatters.formatTestUser,
      ReactiveMongoFormats.objectIdFormats) {

  // List of fields that contain generated identifiers
  val IdentifierFields: Seq[String] = Seq("nino",
                                          "saUtr",
                                          "vrn",
                                          "empRef",
                                          "mtdItId",
  ...
  private def ensureIndex(field: String,
                          indexName: String,
                          isUnique: Boolean = true): Future[Boolean] = {
  ...
  def fetchIndividualByNino(nino: Nino): Future[Option[TestIndividual]] = {
    find("nino" -> nino, "userType" -> UserType.INDIVIDUAL) map (_.headOption map (_.asInstanceOf[
      TestIndividual]))
  }

  def fetchIndividualByShortNino(
      shortNino: NinoNoSuffix): Future[Option[TestIndividual]] = {
    val matchShortNino = Json.obj("$regex" -> s"${shortNino.value}\\w")
    find("nino" -> matchShortNino, "userType" -> UserType.INDIVIDUAL) map (_.headOption map (_.asInstanceOf[
      TestIndividual]))
  }

  def fetchIndividualBySaUtr(saUtr: SaUtr): Future[Option[TestIndividual]] = {
    find("saUtr" -> saUtr, "userType" -> UserType.INDIVIDUAL) map (_.headOption map (_.asInstanceOf[
      TestIndividual]))
  }

  def fetchIndividualByVrn(vrn: Vrn): Future[Option[TestIndividual]] = {
    find("vrn" -> vrn, "userType" -> UserType.INDIVIDUAL) map (_.headOption map (_.asInstanceOf[
      TestIndividual]))
  }

  def fetchOrganisationByEmpRef(
      empRef: EmpRef): Future[Option[TestOrganisation]] = {
    find("empRef" -> empRef.value) map (_.headOption map (_.asInstanceOf[
      TestOrganisation]))
  }

  def fetchOrganisationByVrn(vrn: Vrn): Future[Option[TestOrganisation]] = {
    find("vrn" -> vrn.value, "userType" -> UserType.ORGANISATION) map (_.headOption map (_.asInstanceOf[
      TestOrganisation]))
  }

  def identifierIsUnique(identifier: String): Future[Boolean] = {
    Logger.info(s"Checking tax identifier uniqueness - $identifier")
    val query = Json.obj("$or" -> IdentifierFields.map(identifierField =>
      Json.obj(identifierField -> identifier)))
